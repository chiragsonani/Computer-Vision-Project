...
from vlad_implementation import VLADProcessor

# Global processor instance
processor = VLADProcessor()
...
def dictionary(descriptors, n_clusters):
    ...
    """
    return processor.learn_dictionary(descriptors, n_clusters)
def assignments(descriptors, clusters):
    ...
    """
    return processor.compute_assignments(descriptors, clusters)
...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    """
    encodings = processor.encode_vlad(files, mus, powernorm)
...
def esvm(encs_test, encs_train, C=1000):
    ...
    """
    return processor.apply_esvm(encs_test, encs_train, C)
...
def distances(encs):
    ...
    """
    return processor.compute_distances(encs)
...
if __name__ == '__main__':
    ...
    if not os.path.exists('mus.pkl.gz'):
        ...
        mus = dictionary(descriptors, n_clusters=128)
    ...
    if not os.path.exists(fname) or args.overwrite:
        enc_test = vlad(files_test, mus, args.powernorm, args.gmp, gamma)
    ...
    if not os.path.exists(fname) or args.overwrite:
        enc_train = vlad(files_train, mus, args.powernorm, args.gmp, gamma)
        with gzip.open(fname, 'wb') as fOut:
